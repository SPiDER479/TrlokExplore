...
namespace SpaceGraphicsToolkit.Ocean
{
	public partial class SgtOcean
	{
		...
		public class Batch
		{
			...
			public void AddTriangle(Triangle triangle, float radius)
			{
				var positionA = math.normalize(triangle.PositionA.xyz) * radius;
				var positionB = math.normalize(triangle.PositionB.xyz) * radius;
				var positionC = math.normalize(triangle.PositionC.xyz) * radius;
				var origin    = (int3)math.floor(positionA);
